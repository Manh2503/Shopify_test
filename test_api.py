import requests, json

shop_creds='D:\learn\Shopify\shopify.json'

with open(shop_creds) as f:
    shop_creds = json.load(f)

domain = 'https://development-store253.myshopify.com'

header_values = {'Content-Type': 'application/json', 'X-Shopify-Access-Token':shop_creds['api_token']}

# Bài tập cơ bản về requests (3 ngày)
#1. Tìm hiểu thư viện requests
#2. Tìm hiểu về Shopify API
#a, Đọc hiểu về product, collection, customer, order
# b, create

#i. Collection:
# 1 Smart collection,

def create_smart_collection():
    smart_collection = {
        "smart_collection":{
            "title":"bo suu tap thong minh",
            "rules":[
                 {
                    "column": "title",
                    "relation": "equals",
                    "condition": "Smart"
                }
            ],
            "image": {
                    "attachment": "R0lGODlhAQABAIAAAAAAAAAAACH5BAEAAAAALAAAAAABAAEAAAICRAEAOw==\n",
                    "alt": "iPod"
                    }
        }
    }
    endpoint = '/admin/api/2024-01/smart_collections.json?'
    base_url = domain + endpoint
    r = requests.post(base_url, json=smart_collection, headers=header_values)
    return r
 
# 1 Custom collection.
def create_custom_collection():
    custom_collection = {
        "custom_collection":{
            "title":"bo suu tap tao bang tay",
        }
    }
    endpoint = '/admin/api/2024-01/custom_collections.json'
    base_url = domain + endpoint
    r = requests.post(base_url, json=custom_collection, headers=header_values)
    return r


#ii. Tạo Product
# 1 Product không option,
def create_product_no_option():
    product_new_1 = {
        "product":{
            "title":"quan ao da bong",
            "body_html":"day là quan ao da bong nha"
        }
    }
    endpoint = '/admin/api/2024-01/products.json'
    base_url = domain + endpoint
    r = requests.post(base_url, json=product_new_1, headers=header_values)
    return r


# 1 Product có 2 options
def create_product_have_options():
    product_new_2 = {
        "product": {
            "title":"quan ao da bong moi",
            "body_html":"day là quan ao da bong nha",
            "variants":[{"option1":"màu xanh","option2":"S"},{"option1":"màu xanh","option2":"M"},
                        {"option1":"màu vàng","option2":"S"},{"option1":"màu vàng","option2":"M"},
                        {"option1":"màu tím","option2":"S"},{"option1":"màu tím","option2":"M"}],
            "options": [{
                        "name": "Màu",
                        "values": ["màu xanh", "màu vàng", "màu tim"]},
                        {
                        "name": "Cỡ",
                        "values": ["S", "M"]}
                        ],
        }
    }
    endpoint = '/admin/api/2024-01/products.json'
    base_url = domain + endpoint
    r = requests.post(base_url, json=product_new_2, headers=header_values)
    return r



#iii. Tạo 1 customer
def create_customer():
    customer_new = {
        "customer":{
            "first_name":"The",
            "last_name":"Manh",
            "email":"nguyenmanhthe13@gmail.com",
            "phone":"+84971826184",
            "verified_email": True,
            "addresses":[{
                "address1": "Tran Dang Hoa Son",
                "city": "Ha Noi",
                "province": "ON",
                "phone": "555-1212",
                "zip": "123 ABC",
                "last_name": "Lastnameson",
                "first_name": "Mother",
                "country": "VN"
            }],
            "password":"123456",
            "password_confirmation":"123456",
            "send_email_welcome":False
        }
    }
    endpoint = '/admin/api/2024-01/customers.json'
    base_url = domain + endpoint
    r = requests.post(base_url,json=customer_new, headers=header_values)
    return r


##iv. Tạo 1 đơn hàng với mặt hàng, khách hàng và số lượng của măt hàng
def create_order(variant_id,customer_id,quantity):
    new_order = {
        "order":{"line_items":[{"variant_id":variant_id,"quantity":quantity}],
                 "customer":{"id":customer_id},
                 "financial_status":"pending"}
    }
    endpoint = '/admin/api/2024-01/orders.json'
    base_url = domain + endpoint
    r = requests.post(base_url,json=new_order, headers=header_values)
    return r


# Tạo mới
def create_entity(name_entity, data):
    endpoint = '/admin/api/2024-01/{name_entity}.json'.format(name_entity=name_entity)
    base_url = domain + endpoint
    r = requests.post(base_url,json=data, headers=header_values)

product_new =  {
        "product":{
            "title":"quan ao da bong",
            "body_html":"day là quan ao da bong nha"
        }
    }
create_entity('products',data=product_new)

#c,Update: cập nhật giá, cập nhật ảnh, cập nhật số lượng (qty) cho 2 product vừa tạo.

# cập nhật giá
def update_product(product_id):
    product_update = {
        "product":{
            "id":product_id,
            'variants':[{
                "id": 48964774527279,
                "price":'150',
            }]
    }}
    endpoint = '/admin/api/2024-01/products/{product_id}.json'.format(product_id=product_id)
    base_url = domain + endpoint
    r = requests.put(base_url, json=product_update, headers=header_values)
    return r
    
# thêm ảnh
def create_image_product(product_id):
    image_product = {
        "image":{
            "position":1,
            "attachment":"/APbr48VySrxTO7IgKt2qmKQdJeK8lsFjROG5p/nz7Zg3\\n"
        }
    }
    endpoint = '/admin/api/2024-01/products/{product_id}/images.json'.format(product_id=product_id)
    base_url = domain + endpoint
    r = requests.post(base_url, json=image_product, headers=header_values)
    return r

# cập nhật số lượng của 1 sản phẩm
def update_inventory_quantity(location_id, inventory_item_id, available):
    inventory_level = {"location_id":location_id, "inventory_item_id":inventory_item_id, "available":available}
    endpoint = '/admin/api/2024-01/inventory_levels/set.json'
    base_url = domain + endpoint
    r = requests.post(base_url, json=inventory_level, headers=header_values)
    return r

#  xóa các dữ liệu đã tạo
# Để xóa dữ liệu customer thì truyền name_table = 'customers'
# Để xóa dữ liệu order thì truyền name_table = 'orders'    
# Để xóa dữ liệu product thì truyền name_table = 'prosucts'
# Để xóa dữ liệu custom collection thì truyền name_table = 'custom_collections'
# Để xóa dữ liệu smart collection thì truyền name_table = 'smart_collections'

def delete_all(name_table):
    list = requests.get(domain+'/admin/api/2024-01/{name_table}.json'.format(name_table=name_table),headers=header_values)
    for rec in list.json()['{name_table}'.format(name_table=name_table)]:
        endpoint = '/admin/api/2024-01/{name_table}/{rec_id}.json'.format(name_table=name_table,rec_id=rec['id'])
        base_url = domain + endpoint
        r = requests.delete(base_url, headers=header_values)


